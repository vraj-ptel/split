...
export function SplitSelector({
  ...
  const updatePercentageSplit = (userId, newPercentage) => {
    ...
    const beforeValue=splits.find((s)=>s.userId==userId);
    ...
    if(beforeValue.percentage>newPercentage){
      splits.filter((s)=>s.userId!=userId).map((split) => {
        return {
          ...split,
          percentage: split.percentage+beforeValue.percentage-newPercentage,
          amount: (amount * (split.percentage+beforeValue.percentage-newPercentage)) / 100,
        };
      });
    }
